...
2. **Before starting work**, use the task tools (`TaskCreate`, then `TaskUpdate` / `TaskList` / `TaskGet` to manage them; older harnesses expose this as `TodoWrite`) to create a task list with one item per requirement, plus a final item: "End-to-end verification of every requirement". All items start `pending`.
